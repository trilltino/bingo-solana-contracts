...
mod bingo {
    ...
    pub fn cleanup_room<'info>(
        ctx: Context<'_, '_, '_, 'info, CleanupRoom<'info>>,
        room_id: String,
    ) -> Result<()> {
...
pub struct JoinRoom<'info> {
    ...
    #[account(
        ...
        seeds = [b"room-vault", room.key().as_ref()],
        bump,
        constraint = room_vault.mint == room.fee_token_mint @ BingoError::InvalidTokenMint
    )]
    pub room_vault: Account<'info, anchor_spl::token::TokenAccount>,

    #[account(
        mut,
        constraint = player_token_account.mint == room.fee_token_mint @ BingoError::InvalidTokenMint,
        constraint = player_token_account.owner == player.key() @ BingoError::InvalidTokenOwner
    )]
...
pub struct DeclareWinners<'info> {
    #[account(
        ...
        seeds = [b"room", room.host.as_ref(), room_id.as_bytes()],
        bump = room.bump,
        has_one = host @ BingoError::Unauthorized
    )]
    pub room: Account<'info, Room>,
...
pub struct EndRoom<'info> {
    ...
    #[account(
        mut,
        constraint = platform_token_account.mint == room.fee_token_mint @ BingoError::InvalidTokenMint,
        constraint = platform_token_account.owner == global_config.platform_wallet @ BingoError::InvalidTokenOwner
    )]
    pub platform_token_account: Account<'info, anchor_spl::token::TokenAccount>,

    #[account(
        mut,
        constraint = charity_token_account.mint == room.fee_token_mint @ BingoError::InvalidTokenMint,
        constraint = charity_token_account.owner == room.charity_wallet @ BingoError::InvalidTokenOwner
    )]
    pub charity_token_account: Account<'info, anchor_spl::token::TokenAccount>,

    #[account(
        mut,
        constraint = host_token_account.mint == room.fee_token_mint @ BingoError::InvalidTokenMint,
        constraint = host_token_account.owner == room.host @ BingoError::InvalidTokenOwner
    )]
...
pub struct AddApprovedToken<'info> {
    ...
    /// Token mint to be approved - validated for freeze authority
...
pub struct InitAssetRoom<'info> {
    ...
    /// Using AccountInfo because account doesn't exist yet at instruction invocation time.
    #[account(
        mut,
        seeds = [b"room-vault", room.key().as_ref()],
        bump
...
#[derive(Accounts)]
#[instruction(room_id: String, prize_index: u8)]
pub struct AddPrizeAsset<'info> {
    ...
    /// CHECK: Prize vault PDA - can be uninitialized, will be created in handler if needed.
    /// SECURITY: Handler derives PDA with seeds [b"prize-vault", room.key(), prize_index]
    /// and validates it matches this account. Created via invoke_signed with Token Program
    /// as owner and room PDA as authority. Using UncheckedAccount because it may not exist yet.
    /// Handler validates: (1) correct PDA derivation, (2) correct mint, (3) correct authority.
    #[account(mut)]
    ...
    pub prize_mint: Account<'info, anchor_spl::token::Mint>,
