...
pub fn handler(
    ...
    // Basic validation
    // SECURITY: Validate room_id length (1-32 characters as documented in error message)
    require!(
        room_id.len() >= MIN_ROOM_ID_LENGTH && room_id.len() <= MAX_ROOM_ID_LENGTH,
        BingoError::InvalidRoomId
    );
    require!(max_players > 0 && max_players <= MAX_PLAYER_COUNT, BingoError::InvalidMaxPlayers);
